...
class CustomerAccountCodeNotifier:
   ...
   def _prepare_message(self) -> tuple[str, Any]:
       text = (
           ...
           f"❗️<b>Пожалуйста, следуя инструкции, отправьте новый код, а затем укажите его "
...
class CustomerInvalidEmailNotifier:
    ...
    def _prepare_message(self) -> tuple[str, Any]:
        text = (
            f"🎯 По вашему заказу <b>№{self.order.number}</b> указана <b>неверная почта {self.email}!</b>\n\n"
            f"❗️<b>Пожалуйста, укажите корректный адрес электронной почты в разделе заказа.</b>\n\n"

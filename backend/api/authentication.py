...
class WebAppAuthentication(BaseAuthentication):
    ...
    def authenticate(self, request):
        try:
            ...
            # Проверяем, есть ли у пользователя аватарка и telegram_chat_id
            if user.telegram_chat_id and not user.telegram_avatar_url:
                # Запускаем задачу для получения аватарки
                try:
                    from celery_app import app as celery_app
                    celery_app.send_task("core.update_telegram_avatar", args=[user.id])
                except Exception as avatar_err:
                    logger.warning(f"Не удалось запустить задачу получения аватарки: {avatar_err}")

    ...
    return (
        <div className={styles.popular}>
            {props.data.map((product: IProduct) => {
                ...
                // Вычисляем примерную высоту в зависимости от длины названия
                const titleLength = product.title.length;
                const extraHeight = titleLength > 40 ? Math.ceil((titleLength - 40) / 20) * 20 : 0;
                const itemStyle = extraHeight > 0 ? { minHeight: `calc(265px + ${extraHeight}px)` } : {};

                return (
                    <Link href={`/product/${product.id}`} className={styles.item} key={product.id} style={itemStyle}>
                        ...
                        <div className={styles.bg} style={extraHeight > 0 ? { minHeight: `calc(300px + ${extraHeight}px)` } : {}}>
                            <PopularBg />
                        </div>
                    </Link>
                )
